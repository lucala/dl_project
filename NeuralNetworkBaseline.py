...
class NeuralNetwork(object):
    ...
    def __init__(self, n, m, r, dic_size, l, epochs=5, batchSize=128, loss='sparse_categorical_crossentropy', checkpoint_period=1, custom_metric=None):
        ...
        output_layer = Dense(self._l, activation='softmax', name='output_layer')(merge)

        self._model = Model(inputs=[image_input, word_input], outputs=[output_layer])
        print(self._model.summary())
        if custom_metric != None:
            self._model.compile(loss=self._loss, optimizer='adam', metrics=['accuracy', custom_metric])
        else:
