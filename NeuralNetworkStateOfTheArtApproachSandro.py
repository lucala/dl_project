...
class NeuralNetwork(object):
    ...
    def __init__(self, n, m, r, dic_size, glove_weights, l, k=16, epochs=5, batchSize=128, loss='sparse_categorical_crossentropy',
                 activation='softmax', checkpoint_period=1, wordEmbeddingSize=100, monitor='val_loss'):
        ...
        self._wordEmbeddingSize = wordEmbeddingSize
        ...
        word_embedding = Embedding(self._dic_size, self._wordEmbeddingSize, input_length=self._r, weights=[glove_weights],
        ...
        weighted_sum = Flatten()(Dot(axes=1)([image_input, softmax_1])) #TODO: need to check this, if dimensions are correct, should be self._m
        ...
        output_layer = Dense(int(self._l), activation=activation, name='output_layer')(add_1)
    ...
    def fit(self, images, questions, labels, validation_split=0.3):
        # Split the data
        # self._matrix = matrix
        self._labels = labels
        checkpointer = ModelCheckpoint(filepath="weights_sa/weights-{epoch:02d}.hdf5", verbose=1, save_best_only=False,
        ...
        self._model.fit(x=[images, questions],y=self._labels, batch_size=self._batchSize, epochs=self._epochs, validation_split=validation_split,
